...
tempfile med_groups1
save `med_groups1'

import excel "Data\HFCS\clean_data\interim\Broader medical condition categories.xlsx", cellrange(A1:B32) firstrow clear
rename Medicalcondition med_group

merge 1:m med_group using `med_groups1'
...
collapse (max) value (first) female AGE age_grp3 college college2 college3 isco_1dig isco_1dig_char hand_assign_use2 hand_assign_use2_char Q81 emp_stat weight pass_screener num_limit no_health_problems Broadermedicalcategory, by(prim_key med_group)
...
collapse (max) value (first) female AGE age_grp3 college college2 college3 isco_1dig isco_1dig_char hand_assign_use2 hand_assign_use2_char Q81 emp_stat weight pass_screener num_limit no_health_problems, by(prim_key Broadermedicalcategory)

rename Broadermedicalcategory med_group

replace med_group = subinstr(med_group," ", "_", .)
replace med_group = subinstr(med_group, "/", "_or_",.) 


save "Data\HFCS\clean_data\interim\medcond_g_broad.dta" , replace